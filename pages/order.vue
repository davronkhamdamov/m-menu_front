...
const { data: countries, status, execute } = await useLazyFetch('/api/countries.json', {
  immediate: false
})
...
const sendOrder = async () => {
  ...
  const response = await useApiFetch("/v1/order", locale.value, "POST", payload);
  ...
  const socket = new WebSocket("ws://localhost:8080/ws?table_id=" + store.tableId);
...
const sendFeedback = async () => {
  ...
  const response = await useApiFetch("/v1/feedback", locale.value, "POST", { table_id: store.tableId, star: star.value, region: region.value.name, feedback: feedback.value })
  ...
  <UContainer>
    ...
    <div class="flex justify-center mt-10">
      <UModal v-model:open="openFeedbackModal">
        <UButton class="w-max text-white" color="secondary">{{t('RateService')}}!
        ...
        <template #body>
          <p class="text-center text-xl font-bold">{{ t('rateInstruction') }}</p>
          <br>
        ...
        <template #footer>
          <div class="flex justify-end w-full gap-5">
            <UButton :label="t('later')" color="neutral" variant="outline"
